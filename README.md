...
├── backend/    # Backend — Node + TypeScript service (Express + Anthropic SDK + exceljs)
...
│   • Holds ANTHROPIC_API_KEY (never shipped to the task pane)            │
│   • Streams Claude Opus 4.8 responses (Messages API, adaptive thinking) │
...
> the Claude loop) runs on the backend. The backend will relay PowerPoint tool calls to the task
> pane over a session channel in a later milestone.

### Backend — `backend/`

A small **Node + TypeScript** service (Express). It exists so the Anthropic API key never reaches
...
| `src/routes/chat.ts` | `POST /api/chat`. Takes `{messages}` (Anthropic `MessageParam[]`), opens a streaming Messages API call to **`claude-opus-4-8`** (adaptive thinking, `effort: high`, prompt-cached system prompt) and relays text deltas to the client as SSE events (`text` / `done` / `error`). |
...
| `POST /api/chat` | `{messages: MessageParam[]}` → SSE stream of `event: text` deltas, then `event: done` |
...
- **Git**
- An **Anthropic API key** (for the chat backend)
...
1. In the task pane, type a message → you should see a streamed reply from Claude
...
| Chat shows "backend responded …" error | Backend isn't running or `.env` is missing the API key. Start it with `npm run dev` in `backend/` and check the terminal for the `ANTHROPIC_API_KEY` warning. |
...
- `ANTHROPIC_API_KEY` lives only in `backend/.env` (gitignored); the task pane authenticates to
  the backend, never to Anthropic.
